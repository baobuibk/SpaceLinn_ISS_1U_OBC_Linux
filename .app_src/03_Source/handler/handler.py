...
CMD_REQUEST_RTC_FROM_STM32 = 0x40
CMD_RESPONSE_RTC_FROM_STM32 = 0x41
...
def shutdown():
    try:
        ...
        secret_path = script_dir / "secret.b64"
        ...
        print(f"Error: {e}")
        logger.exception(f"[!] Exception in sudo shutdown now: {e}")
...
# Handlers
def handle_sudo_shutdown_now(payload):
    try:
        print("[CMD] SUDO SHUTDOWN NOW")
        logger.info("[CMD] SUDO SHUTDOWN NOW")

        print("[~] Syncing filesystem...")
        logger.info("[~] Syncing filesystem...")
        subprocess.run(["sync"], check=True)

        print("[OK] Filesystem synced.")
        logger.info("[OK] Filesystem synced.")
        ...
        print(f"[!] Exception in SUDO_SHUTDOWN_NOW handler: {e}")
        logger.exception(f"[!] Exception in SUDO_SHUTDOWN_NOW handler: {e}")
...
def handle_take_image(payload):
    ...
    try:
        ...
        out_file = f"Camera{current_cam_idx}_{ts}.raw"
...
def handle_response_rtc_from_stm32(payload):
    try:
        print("[CMD] RTC FROM STM32")
        if len(payload) != 6:
            print(f"[!] Invalid RTC payload size: {len(payload)}")
            modfsp.send(MODFSP_MASTER_NAK, b'')
            return
...
# Register
modfsp.register_command(CMD_SUDO_SHUTDOWN_NOW, handle_sudo_shutdown_now)
...
modfsp.register_command(CMD_RESPONSE_RTC_FROM_STM32, handle_response_rtc_from_stm32)
...
def request_rtc_from_stm32():
    print("Sending REQUEST RTC command...")
    result = modfsp.send(CMD_REQUEST_RTC_FROM_STM32, b'')
