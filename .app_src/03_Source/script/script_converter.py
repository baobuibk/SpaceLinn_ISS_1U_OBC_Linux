...
class ConfigValidator:
    ...
    def validate_tec_voltage(voltage: float, index: int) -> int:
        ...
        if voltage_mv < 500 or voltage_mv > 2500:
            logger.warning(f"TEC device {index}: voltage {voltage_mv}mV too high (Recommend: 500-2500mV)")
        return voltage_mv
    ...
    def validate_temperature_range(reference: float, min_temp: float, max_temp: float) -> bool:
        """Validate temperature range within [min_temp, max_temp] and ensure all within [0.0°C, 40.0°C]"""
        logger = logging.getLogger(__name__)
        ...
        for label, value in [('Reference', reference), ('Min', min_temp), ('Max', max_temp)]:
            if not (0.0 <= value <= 40.0):
                logger.warning(f"{label} temperature {value}°C is out of allowable range [0.0°C, 40.0°C]")
                return False

        # Check logical range consistency
        if not (min_temp <= reference <= max_temp):
            logger.warning(f"Reference temperature {reference}°C is not within configured range [{min_temp}°C, {max_temp}°C]")
            return False
...
def build_builttostep(beepc1_path: str, 
    ...
    if "data" in sys:
        ...
        run_limit_count = int(sys.get("data", {}).get("run_limit_count", 0))
        ...
        if dls_interval < MIN_INTERVAL or dls_interval > MAX_INTERVAL:
            logger.warning(f"dls_interval {dls_interval}s is out of range ({MIN_INTERVAL}–{MAX_INTERVAL}s). Clamping to valid range.")
        ...
        if cam_interval < MIN_INTERVAL or cam_interval > MAX_INTERVAL:
            logger.warning(f"cam_interval {cam_interval}s is out of range ({MIN_INTERVAL}–{MAX_INTERVAL}s). Clamping to valid range.")
    ...
    for d in enabled_tec_devices:
        ...
        if voltage_mv < 500 or voltage_mv > 2500:
            stats["warnings"] += 1
        if voltage_mv > 4000:
            logger.error(f"TEC device: voltage {voltage_mv}mV > 4000mV")
            voltage_mv = 4000
            stats["errors"] += 1
        ...
        if prof["type"] == "feedback":
            ...
            if tecs:
                ...
                if interval < 0 or interval > 60:
                    ...
                    interval = max(0, min(interval, 60))  # Clamp to 0–60
                    stats["warnings"] += 1
    ...
    for seq in dls.get("sequences", []):
        ...
        result["dls_routine"]["steps"].extend([
            ...
            {"action": "set_position", "parameters": {"position": int(seq["ld_pd_id"])}},
...
#----------------------------------------------------------------------------------------------------
    ...
    if ext_laser_output_path and stats["errors"] == 0:
        ...
        try:
            # Lấy các trường "index", "ld_id", "ld_current" từ mỗi sequence
            ext_laser_sequences = []
    ...
    # --- NEW: Save background step file ---
    if background_output_path and stats["errors"] == 0:
        logger.info(f"Saving background config to {background_output_path}...")
        try:
            background = {
                "init": {
                    "steps": [
            ...
            other_init_steps = [
                step for step in result["init"]["steps"]
                if step.get("action") not in {"clear_profile", "set_system"}
            ]
            background["init"]["steps"].extend(other_init_steps)
            
            # Copy DLS steps nhưng intensity = 0
            background["dls_routine"]["steps"].append({
                "action": "set_dls_interval",
                "parameters": {"interval": int(dls.get("interval", 3600))}
            })

            for seq in dls.get("sequences", []):
                background["dls_routine"]["steps"].extend([
                    {"action": "set_laser_intensity", "parameters": {"intensity": 0}},
                    {"action": "set_position", "parameters": {"position": int(seq["ld_pd_id"])}},
                    {"action": "start_sample_cycle", "parameters": {}},
                    {"action": "obc_get_sample", "parameters": {}}
                ])

            background["cam_routine"]["steps"].append({
                "action": "set_camera_interval",
                "parameters": {"interval": cam_interval}
            })
            ...
            # Copy CAM steps nhưng intensity = 0
            for seq in cam_routine.get("sequences", []):
                ld_id = seq["ld_id"]
                if isinstance(ld_id, list):
                    bitmask = 0
                ...
                background["cam_routine"]["steps"].extend([
                    {"action": "set_ext_laser_intensity", "parameters": {"intensity": 0}},
                    {"action": "turn_on_ext_laser", "parameters": {"position": bitmask}},
                    {"action": "set_camera_position", "parameters": {"cis_id": int(seq["cis_id"])}},
                    {"action": "take_img_with_timeout", "parameters": {}},
                    {"action": "delay", "parameters": {"duration": 3000}},
            ...
            with open(background_output_path, "w") as f:
                json.dump(background, f, indent=4)
            logger.info(f"Successfully saved background config to {background_output_path}")
        except Exception as e:
            logger.error(f"Failed to save background file '{background_output_path}': {e}")
            stats["errors"] += 1
...
def main():
    ...
    output_background = args.output_dir / "beepc1_background.json"
